...
V1.1 是在三个真实案例验证后形成的加法式升级，保持 V1.0 数据可读，不删除或重命名既有字段。

本次核心变化：

- DataPoint 新增可选 `measurement_context`，表达币种、数量级、报告标准、归属范围和合并范围；
- Claim 新增单值 `claim_dimension`，区分主张形式和分析维度；
- Provenance 新增结构化 `derivation_links`，区分摘要、转载、计算、翻译、提取和推断；
- Evidence 独立性在聚合层按 `target + role + independence_group` 去重；
- Traceability 中非依赖审计引用不再产生阻塞性 dangling error；
- V1.0 与 V1.1 采用双版本读取、混合存储和显式迁移。

V1.1 不引入自动投资决策、自动 Claim 拆分、LLM 抽取或 WQRS 专属财务规则。
...
因此，Aurora V1.1 必须维持并增强统一对象模型，并明确：
...
| `schema_version` | string | 是 | 当前写入版本 `1.1`；兼容读取 `1.0` |
...
`DataPoint` 表示具有明确数值、单位、时间、来源和测量口径的数据。
...
| 字段 | 含义 | 示例 |
|---|---|---|
| `measurement_kind` | 数值性质 | monetary、ratio、count |
| `currency` | 三位大写币种代码 | CNY、USD |
| `scale_multiplier` | 原始显示值的乘数 | 1、1000、100000000 |
| `reporting_standard` | 报告或计量标准 | CAS、IFRS |
| `attribution_scope` | 数值归属口径 | attributable_to_parent |
| `consolidation_scope` | 合并范围 | consolidated、parent_only |

`MeasurementContext` 是可选嵌套对象。非金融数据不需要填写财务专属口径；V1.0 数据读取时使用保守默认值，不自动猜测币种、会计准则或归属范围。
...
`Claim` 表示某个主体提出的、可以被支持、反驳、限定或验证的主张。Claim 不等于事实。
...
### ClaimType 与 ClaimDimension

`claim_type` 回答“说话者在做什么形式的主张”：
...
两条 Claim 可能观点不同，也可能只是分析维度不同。系统不得仅因结论语气不同就自动标记为相互反驳。

### Claim 原子性

一条 Claim 应只表达一个可被支持、反驳或验证的主要判断。V1.1 使用警告式 Linter 提示复合主张，但不通过硬性校验阻止保存。自动拆分留给后续 Understanding 层。

### 观点归属纪律

每条 Claim 必须明确谁说的、何时说的、来源位置、是否直接引用以及是否由 Aurora 推断。
...
每个派生对象都必须包含 `provenance`。V1.1 在保留 `origin_object_ids` 的同时增加结构化派生关系。
...
{
  "origin_type": "derived",
  "origin_object_ids": ["doc_original"],
  "derivation_links": [
    {
      "object_id": "doc_original",
...
- `origin_object_ids` 继续保留；
- V1.0 的 origin ID 在内存适配时映射为最保守的 `derived_from`；
- 不根据文本自动猜测 `summarizes`、`calculated_from` 等关系；
- 读取 V1.0 不自动写回数据库；
- `processing_run_id` 是审计引用，缺失时可以产生提示，但不破坏认知链完整性。

### 来源链原则

```text
PersonalOpinion / OutputArtifact
→ Insight
→ KnowledgeObject
...
任何强依赖环节断裂都应降低对象可用等级；非依赖审计引用不应制造阻塞性噪音。
...
```text
docs/03_data_model/01_Aurora_核心对象模型与知识标准_V1.1.md
```
...
4. 人工制作黄金标准答案
5. 验证对象模型是否足够表达
6. 根据真实案例完成 V1.1 加法式升级
7. 冻结 MVP Schema V1.1
